...
function getAccountOverview(data) {
	...
	//总资产
	totalMoney = toDecimal2(Math.floor((account.totalMoney) * 100) / 100);
	$(".mspan1").empty().append(totalMoney);

	// 累计收益
	totalProfit = toDecimal2(account.totalProfit);
	$(".mspan2").empty().append(totalProfit);

	// 昨日收益
	totalYesterdayProfit = toDecimal2(account.totalYesterdayProfit);
...
function myMoney(data) {
	...
	// 总收益 = 历史收益（昨日收益）+ 代收收益
	totalProfit = toDecimal2(Math.floor((Number(dssy) + Number(lssy)) * 100) / 100);
...
function getMFList(index) {
	...
	function getMyInviteList(data) {
		...
		$(".invitationLink #text1").empty().append("https://www.qihangjf.com/register.html?invitationMyCode=" + invitationCode)
